#!/usr/bin/env python
"""
Evaluate student (or any HF DNA encoder) on the Nucleotide Transformer (NT) downstream benchmark
stored as per-task Parquet files.
...
Each Parquet file is expected to contain columns:
    - sequence: str (DNA sequence)
    - label: int (class label)
    - name: str (optional, not used)
    - task: str (optional; if multiple tasks are present in one Parquet, we can split by it)

Protocol (paper-style, MCC-centered):
    - Build K stratified folds on TRAIN set.
    - For each fold:
        * Train on train\fold
        * Select the best checkpoint by MCC on fold (validation)
        * Evaluate the selected checkpoint on the provided TEST set
...
def _read_parquet_any(path: Path):
    ...
    We keep this as a small helper because parquet readers depend on optional backends
    (pyarrow / fastparquet). If your environment lacks them, install one of:
        pip install pyarrow
    ...
    try:
        ...
        raise RuntimeError(
            ...
            f"Hint: install a parquet backend (pyarrow or fastparquet)."
        ) from e


def find_candidate_parquet_tasks(root: Path) -> List[Path]:
    """
    Find immediate subdirectories that contain both train.parquet and test.parquet.
    """
    out = []
    for p in sorted(root.iterdir()):
        if p.is_dir() and (p / "train.parquet").exists() and (p / "test.parquet").exists():
            out.append(p)
    return out


def _standardize_columns(df, seq_col: str = "sequence", label_col: str = "label") -> Any:
    """
    Ensure sequence/label columns exist (with fallbacks).
    """
    cols = {c.lower(): c for c in df.columns}
    # Sequence fallbacks
    if seq_col not in df.columns:
        if "seq" in cols:
            df = df.rename(columns={cols["seq"]: seq_col})
        elif "sequence" in cols:
            df = df.rename(columns={cols["sequence"]: seq_col})
        else:
            raise KeyError(f"Cannot find a sequence column in parquet columns: {list(df.columns)}")
    # Label fallbacks
    if label_col not in df.columns:
        if "labels" in cols:
            df = df.rename(columns={cols["labels"]: label_col})
        elif "y" in cols:
            df = df.rename(columns={cols["y"]: label_col})
        elif "label" in cols:
            df = df.rename(columns={cols["label"]: label_col})
        else:
            raise KeyError(f"Cannot find a label column in parquet columns: {list(df.columns)}")
    return df


def load_task_splits_from_dir(
    task_dir: Path,
    group_by_task_column: bool = True,
    seq_col: str = "sequence",
    label_col: str = "label",
    task_col: str = "task",
) -> Dict[str, Dict[str, Any]]:
    """
    Load {task_name: {"train": df_train, "test": df_test}}.

    If group_by_task_column=True and both parquet files contain a task column,
    we split the data by unique df[task_col] values (intersection of train/test tasks).
    Otherwise, we treat the whole directory as a single task.
    """
    df_tr = _standardize_columns(_read_parquet_any(task_dir / "train.parquet"), seq_col, label_col)
    df_te = _standardize_columns(_read_parquet_any(task_dir / "test.parquet"), seq_col, label_col)

    # Drop rows with missing seq/label
    df_tr = df_tr.dropna(subset=[seq_col, label_col]).reset_index(drop=True)
    df_te = df_te.dropna(subset=[seq_col, label_col]).reset_index(drop=True)

    # Optionally split by task column if it exists and is meaningful
    if group_by_task_column and (task_col in df_tr.columns) and (task_col in df_te.columns):
        tr_tasks = sorted(set(df_tr[task_col].astype(str).unique().tolist()))
        te_tasks = sorted(set(df_te[task_col].astype(str).unique().tolist()))
        common = [t for t in tr_tasks if t in set(te_tasks)]
        if len(common) > 1:
            out = {}
            for t in common:
                out_name = f"{task_dir.name}::{t}"
                out[out_name] = {
                    "train": df_tr[df_tr[task_col].astype(str) == t].reset_index(drop=True),
                    "test": df_te[df_te[task_col].astype(str) == t].reset_index(drop=True),
                }
            return out

    # Default: one task per folder
    return {task_dir.name: {"train": df_tr, "test": df_te}}
...
class ParquetClassification(Dataset):
    """
    In-memory dataset holding (sequence, label_int) pairs.
    Labels are assumed to be already mapped to contiguous [0..C-1].
    """
    def __init__(self, sequences: List[str], labels: List[int]):
        assert len(sequences) == len(labels)
        self.seqs = [str(s) for s in sequences]
...
def build_datasets_from_dfs(df_tr, df_te, seq_col="sequence", label_col="label") -> Tuple[ParquetClassification, ParquetClassification, Dict[Any, int]]:
    """
    Build train/test Dataset objects with a consistent label remapping.
    """
    y_tr_raw = df_tr[label_col].tolist()
    y_te_raw = df_te[label_col].tolist()

    # Build a label -> index mapping (sorted for determinism)
    uniq = sorted(set([int(x) for x in y_tr_raw] + [int(x) for x in y_te_raw]))
    lab2id = {lab: i for i, lab in enumerate(uniq)}

    y_tr = [lab2id[int(x)] for x in y_tr_raw]
    y_te = [lab2id[int(x)] for x in y_te_raw]

    ds_tr = ParquetClassification(df_tr[seq_col].tolist(), y_tr)
    ds_te = ParquetClassification(df_te[seq_col].tolist(), y_te)
    return ds_tr, ds_te, lab2id
...
class StudentEncoder(BaseEncoder):
    ...
    This implementation is intentionally robust across training wrappers:
    ...
    Notes:
      - We keep `strict=False` to tolerate minor version drift.
      - If you see thousands of missing/unexpected keys, it almost always means a prefix mismatch
        (e.g., checkpoint keys are "backbone.xxx" but model expects "xxx").
    """
    def __init__(
        self,
        ckpt_path: str,
        ...
        def _extract_state_dict(obj: Any) -> Dict[str, torch.Tensor]:
            """Extract a state_dict from common checkpoint wrappers."""
            if isinstance(obj, dict):
                for key in ("state_dict", "model", "student_state_dict", "net", "weights"):
                    if key in obj and isinstance(obj[key], dict):
                        return obj[key]
                # raw state_dict: string->Tensor
        ...
        def _maybe_strip_prefix(sd: Dict[str, torch.Tensor], prefix: str) -> Dict[str, torch.Tensor]:
            """
            If a large fraction of keys start with `prefix`, strip it.

            This is crucial for stage-2 checkpoints that often save the encoder under "backbone.".
            """
            if not sd:
                return sd
            keys = list(sd.keys())
        ...
        # Core hyper-params (fallbacks match your training defaults)
        ...
        def _looks_like_moe(sd: Dict[str, torch.Tensor]) -> bool:
            # Heuristic: keys introduced by MoE feedforward/router modules
            for k in sd.keys():
                if ".moe." in k or ".router." in k or "router.proj" in k or ".experts." in k:
                    return True
            return False

        # use_moe = bool(force_moe or moe_cfg.get("use_moe", False) or _looks_like_moe(state))
        ...
        # If moe_cfg exists, prefer it for hyperparams; otherwise use CLI defaults.
        if moe_cfg:
        ...
        # Instantiate with signature-aware kwargs for backward compatibility.
        import inspect
        ...
        if not random_init:
            ...
            if verbose:
                print(
                    f"[student{'+moe' if use_moe else ''}] "
                    f"use_moe={use_moe} d_model={d_model} n_layers={n_layers} "
                    f"expand={expand} d_state={d_state} d_conv={d_conv}"
                )
                if unexpected:
    ...
    def encode(self, seqs: List[str], max_len: int, amp: bool = False) -> torch.Tensor:
        ...
        # Pass compute_logits=False if supported (saves memory/compute in downstream tasks)
        import inspect
...
def inject_lora(model: nn.Module, include_kw: List[str], exclude_kw: List[str],
                r: int, alpha: int, dropout: float, verbose: bool = True) -> int:
    """
...
def run_linear_probe_cv(
    ...
    labels_train = [y for _, y in ds_train]
    n_classes = max(labels_train + [y for _, y in ds_test]) + 1
...
def run_lora_cv(
    ...
    # Freeze base weights (LoRA will keep its own trainable params)
    ...
    if not include_kw:
        if is_student:
            # StudentMamba2 is not a Transformer; inject broadly (all Linear) except excluded names.
            include_kw = []
    ...
    labels_all = [y for _, y in ds_train]
    n_classes = max(labels_all + [y for _, y in ds_test]) + 1
    ...
    for f, (tr_idx, va_idx) in enumerate(folds, 1):
        ...
        # Trainable params = LoRA params (created inside LoRALinear) + head params
...
def main():
    ap = argparse.ArgumentParser(description="NT-benchmark evaluation from train/test.parquet (MCC + macro-F1)")
    ...
    ap.add_argument("--datasets_root", type=str, required=True, help="Root directory of NT benchmark tasks.")
    ap.add_argument("--datasets", type=str, default="auto", help="Comma-separated task folder names; or 'auto'.")
    ...
    # Parquet splitting
    ap.add_argument("--group_by_task", action="store_true",
                    help="If parquet contains a 'task' column with >1 unique values, evaluate each task separately.")
    ...
    ap.add_argument("--student_random_init", action="store_true",
                    help="For --encoder_type in {student,student_moe}: build the same architecture but DO NOT load checkpoint weights (random init baseline).")
    ...
    if args.datasets.lower() == "auto":
        ds_dirs = find_candidate_parquet_tasks(root)
    else:
        names = [s.strip() for s in args.datasets.split(",") if s.strip()]
        ds_dirs = [root / n for n in names]
    ...
    # Load all tasks (folder-level, optionally split by "task" column)
    all_task_splits: Dict[str, Dict[str, Any]] = {}
    for d in ds_dirs:
        all_task_splits.update(
            load_task_splits_from_dir(d, group_by_task_column=args.group_by_task)
        )

    all_results = {}
    for task_name, splits in all_task_splits.items():
        ds_tr, ds_te, lab2id = build_datasets_from_dfs(splits["train"], splits["test"])
        print(f"\n=== {args.mode} on: {task_name} | K={args.k_folds} (select by MCC; also report macro-F1) ===")
        print(f"[data] train={len(ds_tr)} test={len(ds_te)} n_classes={len(lab2id)}")
        ...
        print(f"[SUMMARY-MCC] {task_name} | VAL mean={vm:.4f} std={vs:.4f} median={vmed:.4f}")
        print(f"[SUMMARY-MCC] {task_name} | TEST mean={tm:.4f} std={ts:.4f} median={tmed:.4f}")
        ...
        print(f"[SUMMARY-F1 ] {task_name} | VAL mean={vf1m:.4f} std={vf1s:.4f} median={vf1med:.4f}")
        print(f"[SUMMARY-F1 ] {task_name} | TEST mean={tf1m:.4f} std={tf1s:.4f} median={tf1med:.4f}")
    ...
    print("\n[Done] NT-benchmark evaluation completed.")
